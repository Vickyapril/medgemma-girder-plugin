        ...
        restRequest({
            ...
            if (status === 'success') {
                ...
                events.trigger('g:alert', {
                    icon: 'ok',
                    text: 'MedGemma triage pipeline completed successfully.',
                    type: 'success',
                    timeout: 6000
                });
                return;
            ...
            if (status === 'failed') {
                ...
                events.trigger('g:alert', {
                    icon: 'cancel',
                    text: 'MedGemma pipeline failed. Check Airflow logs.',
                    type: 'danger',
                    timeout: 8000
                });
                return;
...
ItemView.prototype.events[`click .${ACTION_CLASS}`] = function (event) {
    ...
    restRequest({
        ...
        events.trigger('g:alert', {
            icon: 'ok',
            text: `MedGemma triage pipeline started. DAG: ${dagId}, Run ID: ${runId}`,
            type: 'success',
            timeout: 8000
        });
